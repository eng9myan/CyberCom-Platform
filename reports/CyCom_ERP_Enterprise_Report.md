...
**Decision (2026-07-04, revised same day):** Product 7 (CyCom ERP) is a **custom build**, extending the Django app already scaffolded at `CyberCom-Platform/backend/products/cycom/` (finance gl/ap/ar/budgeting/cost_accounting/treasury, procurement, hr, payroll, inventory, assets incl. fleet/biomedical/maintenance, crm, bi, retail/pos). **No Odoo runtime ships in production, and no Odoo/LGPL source is copied.** Odoo 19 is a functional spec to recreate clean-room — same capability, our own code — specifically to avoid any Odoo copyleft/licensing exposure. This keeps the platform's original "no Odoo in production" rule intact for all products, including CyCom ERP.

**Reference sources for the clean-room rebuild (behavior/requirements only, never copy Odoo/LGPL code verbatim):**
- `eng9myan/CyCom` — Odoo 19 Community ("Cycom 19" in-repo) + Next.js 16 wizard UI. Mine the wizard UX design and the 10 setup-wizard doctrine; do not deploy its Odoo container.
- `D:\Anabtawi-Group-main.zip` (extracted to `_reference/Anabtawi-Group-main/`) — the original, pre-rename source of the 75 custom Odoo modules bundled in `eng9myan/CyCom`. This is real production business logic (payroll overtime rules, POS pledge/advance-order flows, attendance/biometric handling, warehouse restrictions, discount-exception approvals, etc.) written by a real Odoo implementer for a real client (Anabtawi Group). Read the *behavior* out of these modules — what business rule each one encodes — and reimplement that rule as native Django/DRF logic in `products/cycom/`. These modules extend Odoo's ORM/framework directly, so a line-for-line port isn't possible anyway (Django has a different model/view/ORM shape) — this is a genuine rewrite, not a copy, by construction.
