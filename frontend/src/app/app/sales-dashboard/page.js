...
export default function SalesDashboardPage() {
  ...
  const calculateMetrics = async () => {
    ...
    try {
      // 1. Fetch Orders
      const orderRes = await fetch("http://localhost:8000/api/v1/sales/orders/", {
      ...
      const oppRes = await fetch("http://localhost:8000/api/v1/sales/opportunities/", {
        headers: { "Authorization": `Bearer ${token}`, "X-Tenant-ID": tenantId }
      });
      let fetchedOpps = [];
      ...
      // 3. Fetch Leads
      const leadRes = await fetch("http://localhost:8000/api/v1/sales/leads/", {
        headers: { "Authorization": `Bearer ${token}`, "X-Tenant-ID": tenantId }
      });
      let fetchedLeads = [];
      ...
      const commRes = await fetch("http://localhost:8000/api/v1/sales/communications/", {
        headers: { "Authorization": `Bearer ${token}`, "X-Tenant-ID": tenantId }
      });
      let fetchedComms = [];
  ...
  return (
    <div className="space-y-8 text-zinc-100 animate-fade">
      
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-xl font-bold tracking-tight text-white">Sales & CRM Analytics</h1>
          <p className="text-xs text-zinc-400">Executive dashboard detailing revenue cycles, average order values, and conversion pipelines.</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        {/* Total Revenue */}
        <div className="bg-zinc-900/60 border border-zinc-800 p-6 rounded-2xl flex items-center justify-between hover:border-zinc-700 transition">
          <div className="space-y-1">
            <span className="text-[10px] text-zinc-500 uppercase tracking-widest font-bold block">Fulfillment Revenue</span>
            <div className="text-2xl font-extrabold font-mono text-orange-500">{data.revenue.toFixed(2)} JOD</div>
            <span className="text-[9px] text-zinc-400 flex items-center gap-1 font-bold">
              <TrendingUp className="w-3 h-3 text-emerald-400" />
              <span>+12.4% vs last week</span>
            </span>
          </div>
          <div className="w-10 h-10 bg-zinc-950 rounded-xl border border-zinc-850 flex items-center justify-center">
        ...
        <div className="bg-zinc-900/60 border border-zinc-800 p-6 rounded-2xl flex items-center justify-between hover:border-zinc-700 transition">
          <div className="space-y-1">
            <span className="text-[10px] text-zinc-500 uppercase tracking-widest font-bold block">Average Order Value</span>
            <div className="text-2xl font-extrabold font-mono text-zinc-100">{data.aov.toFixed(2)} JOD</div>
            <span className="text-[9px] text-zinc-400 flex items-center gap-1 font-bold">
        ...
        <div className="bg-zinc-900/60 border border-zinc-800 p-6 rounded-2xl flex items-center justify-between hover:border-zinc-700 transition">
          <div className="space-y-1">
            <span className="text-[10px] text-zinc-500 uppercase tracking-widest font-bold block">CRM Deal Pipeline</span>
        ...
        <div className="bg-zinc-900/60 border border-zinc-800 p-6 rounded-2xl flex items-center justify-between hover:border-zinc-700 transition">
          <div className="space-y-1">
            <span className="text-[10px] text-zinc-500 uppercase tracking-widest font-bold block">Deal Win Rate</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-zinc-900/60 border border-zinc-800 p-6 rounded-2xl lg:col-span-2 space-y-6">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] text-zinc-450 uppercase tracking-widest font-bold">Leads-to-Opportunities Pipeline Funnel</span>
            </div>
            <span className="text-[9px] bg-zinc-950 border border-zinc-850 px-2 py-0.5 rounded-full text-zinc-500 font-bold font-mono">
              Real-time Analysis
            </span>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1">
              ...
              <div className="h-3 w-full bg-zinc-950 rounded-full overflow-hidden">
                <div className="h-full bg-zinc-700 rounded-full transition-all duration-500" style={{ width: "100%" }}></div>
              </div>
            </div>
        ...
        <div className="bg-zinc-900/60 border border-zinc-800 p-6 rounded-2xl space-y-6">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] text-zinc-450 uppercase tracking-widest font-bold">Activity Indices</span>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between border-b border-zinc-850 pb-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-zinc-300">CRM Channels logged</span>
              </div>
              <span className="text-xs font-mono font-bold text-zinc-100">{data.commCount} logs</span>
            </div>

            <div className="flex items-center justify-between border-b border-zinc-850 pb-3">
              <div className="flex items-center gap-2">
                <span className="w-2.5 h-2.5 rounded-full bg-sky-400"></span>
                <span className="text-xs font-bold text-zinc-300">Open Opportunities</span>
              </div>
              <span className="text-xs font-mono font-bold text-zinc-100">{data.oppCount} active</span>
            </div>

            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <span className="w-2.5 h-2.5 rounded-full bg-emerald-400"></span>
                <span className="text-xs font-bold text-zinc-300">Conversion Won index</span>
              </div>
              <span className="text-xs font-mono font-bold text-zinc-150">High Velocity</span>
            </div>
